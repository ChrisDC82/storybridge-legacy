...
function SubmissionReview({ submission, note, noteError, onNoteChange, onModerate }: { submission: LocalSubmission; note: string; noteError?: string; onNoteChange: (value: string) => void; onModerate: (status: ModerationStatus) => void }) {
  ...
  return (
    <article className="admin-submission-card">
      ...
      <details className="review-details"><summary>Open human review</summary><div className="review-comparison"><section><h4>1. Original rough memory</h4><p>{submission.story.originalRoughMemory}</p></section><section><h4>2. Guidance record</h4><p>Guided Story Mode: <strong>{submission.guidedStoryMode}</strong>. Detailed question responses were not stored in Stage 7.</p></section><section><h4>3. Final contributor-approved draft</h4><p>{submission.story.approvedDraft}</p></section><section><h4>4. Accuracy reminder</h4><p>Approximate date: {submission.story.approximateEventYear}. Treat memory as an account, not verified fact. Check permissions, uncertainty and supporting sources before any future publication.</p></section></div>
