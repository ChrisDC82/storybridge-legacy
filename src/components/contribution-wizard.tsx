    ...
    <section className="wizard-panel" aria-labelledby="wizard-title">
      ...
      {step === 1 && (
        <div className="wizard-step-panel">
          ...
          <div className="form-grid">
            <Field id="fullName" label="Full name" error={errors.fullName}><input id="fullName" value={form.fullName} onChange={(event) => updateField("fullName", event.target.value)} maxLength={100} required autoComplete="name" /></Field>
            <Field id="email" label="Contact email" error={errors.email} hint="Stored locally for this demonstration and never displayed publicly."><input id="email" type="email" value={form.email} onChange={(event) => updateField("email", event.target.value)} maxLength={254} required autoComplete="email" /></Field>
            <Field id="relationship" label="Relationship to the institution" error={errors.relationship}><select id="relationship" value={form.relationship} onChange={(event) => updateField("relationship", event.target.value as ContributionFormData["relationship"])} required><option value="">Select one</option>{institutionRelationships.map((item) => <option key={item}>{item}</option>)}</select></Field>
            <Field id="schoolPeriod" label="School period or connection" error={errors.schoolPeriod} hint="For example: Form 4, 2008–2010, or community partner."><input id="schoolPeriod" value={form.schoolPeriod} onChange={(event) => updateField("schoolPeriod", event.target.value)} maxLength={40} required /></Field>
            <Field id="storyTitle" label="Story title" error={errors.storyTitle}><input id="storyTitle" value={form.storyTitle} onChange={(event) => updateField("storyTitle", event.target.value)} maxLength={120} required /></Field>
            <Field id="eventYear" label="Event year" error={errors.eventYear} hint='Enter a four-digit year or “Not sure”.'><input id="eventYear" value={form.eventYear} onChange={(event) => updateField("eventYear", event.target.value)} maxLength={20} required inputMode="numeric" /></Field>
            <Field id="category" label="Category" error={errors.category}><select id="category" value={form.category} onChange={(event) => updateField("category", event.target.value as ContributionFormData["category"])} required><option value="">Select one</option>{legacyCategories.map((item) => <option key={item}>{item}</option>)}</select></Field>
            <Field id="imageUrl" label="Optional image link" error={errors.imageUrl} hint="Use an http:// or https:// link only; no upload occurs."><input id="imageUrl" type="url" value={form.imageUrl} onChange={(event) => updateField("imageUrl", event.target.value)} maxLength={500} /></Field>
            <Field id="roughMemory" label="Rough memory" error={errors.roughMemory} hint="80–5,000 characters. Write what you remember; uncertainty is welcome." wide><textarea id="roughMemory" value={form.roughMemory} onChange={(event) => updateField("roughMemory", event.target.value)} minLength={80} maxLength={5000} required rows={9} /><span className="character-count">{form.roughMemory.length}/5,000</span></Field>
          </div>
      ...
      {step === 3 && (
        <div className="wizard-step-panel">
          ...
          <Field id="finalDraft" label="Final story draft" error={errors.finalDraft} hint="80–7,000 characters." wide><textarea id="finalDraft" value={finalDraft} onChange={(event) => { setFinalDraft(event.target.value); setErrors((current) => ({ ...current, finalDraft: undefined })); }} minLength={80} maxLength={7000} required rows={16} /></Field>
          <div className={`check-field${errors.draftReviewed ? " has-error" : ""}`}><input id="draftReviewed" type="checkbox" checked={draftReviewed} onChange={(event) => { setDraftReviewed(event.target.checked); setErrors((current) => ({ ...current, draftReviewed: undefined })); }} /><div><label htmlFor="draftReviewed">I reviewed this draft and confirm that it represents my account.</label>{errors.draftReviewed && <p className="field-error">{errors.draftReviewed}</p>}</div></div>
      ...
      {step === 4 && (
        <div className="wizard-step-panel">
          ...
          <div className={`check-field${errors.consent ? " has-error" : ""}`}><input id="consent" type="checkbox" checked={form.consent} onChange={(event) => updateField("consent", event.target.checked)} /><div><label htmlFor="consent">I consent to storing this fictional demonstration submission in this browser for human moderation review.</label>{errors.consent && <p className="field-error">{errors.consent}</p>}</div></div>
